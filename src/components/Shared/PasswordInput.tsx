...
const PasswordInput = <T extends Record<string, any>>({
    ...
    return (
        <div className="w-full mx-auto">
            <label htmlFor={name} className={`mb-1.5 font-figtree block font-medium text-black ${!isLarge ? "text-sm" : ""}`}>
                {label}
            ...
            <div className="relative">
                <input
                    ...
                    className={`pr-10 w-full ${!isLarge ? "rounded-sm" : "rounded"} border bg-zinc-100 py-2.5 px-4 text-black outline-none transition disabled:cursor-default disabled:bg-whiter dark:bg-form-input font-figtree placeholder:font-poppins ${errors?.[name]
                        ? "border-danger"
                        : "dark:text-white border-stroke"
                        } ${!isLarge ? "text-sm" : ""}`}
                />
